...
void turn_pig_latin(char* w) {
  word_length=0;
	...
	//Finding out whether all the characters are capital letters
		...
		is_all_capital = (is_capital_character(w[0]) && is_capital_character(w[word_length-1]));
		int vowel_index = 0;
		//loop counter
		int i = 0;
		...
		int m = 0; //index counter
		int o = vowel_index;
        // prints words after vowel
		while (o < word_length) {
			pig_latin[n] = w[o];
			n++;
			o++;
		...
		// checks whether all are capital and changes cases
        if(is_capital && (is_all_capital==0)){
...
int find_a_word(char* inp, char* w) {
	...
	input_index++;
		...
		if (is_valid_ch) {
		    ...
		    w[0] = '\0'; // how to seperate the two words two words keep on appending
